...
class ClsEvaluator(HookBase):
    ...
    def eval(self):
        ...
        for i, input_dict in enumerate(self.trainer.val_loader):
            ...
            self.trainer.storage.put_scalar("val_DA", DA)
        ...
        DA = self.trainer.storage.history("DA").mean
        ...
        self.trainer.logger.info(
            "Val result: mIoU/mAcc/allAcc/DA {:.4f}/{:.4f}/{:.4f}/{:.4f}.".format(
                m_iou, m_acc, all_acc, DA
            )
        )
        ...
        if self.trainer.writer is not None:
            ...
            self.trainer.writer.add_scalar("val/DA", DA, current_epoch)
        self.trainer.logger.info("<<<<<<<<<<<<<<<<< End Evaluation <<<<<<<<<<<<<<<<<")
        self.trainer.comm_info["current_metric_value"] = DA  # save for saver
        self.trainer.comm_info["current_metric_name"] = "DA"  # save for saver
...
class SemSegEvaluator(HookBase):
    ...
    def eval(self):
        ...
        for i, input_dict in enumerate(self.trainer.val_loader):
            ...
            # 可视化 pred 和 segment
            if i == 0:  # 只可视化第一个 batch（可以调整）
                # 提取坐标和标签
                coord = input_dict["coord"].cpu().numpy()  # 点的坐标 (x, y, t)
                pred_np = pred.cpu().numpy() 
                segment_np = segment.cpu().numpy()         # 真实的标签
                p = input_dict["feat"][:,3].cpu().numpy()          # 原始点的正负事件
                ...
                def render(x, y, p, mask, shape):
                    img = np.full(shape=tuple(shape) + (3,), fill_value=255, dtype="uint8")  # 初始化白色背景
                    valid = (y >= 0) & (y < shape[0]) & (x >= 0) & (x < shape[1]) & mask  # 过滤有效点
                    x, y, p = x[valid], y[valid], p[valid]
                    x = x.astype(int)
                    y = y.astype(int)
                    # print(x, y, p)
                    # print(f"x type: {type(x)}, y type: {type(y)}, p type: {type(p)}")
                    img[y, x, :] = 0 
                    img[y, x, 0] = 255 * (p > 0)  # 红色通道：正事件
                    img[y, x, 1] = 255 * (p < 0)  # 绿色通道：负事件
                    return img

                # 生成原始点云图像
                original_img = render(x, y, p, np.ones_like(p, dtype=bool), (352, 460))

                # 生成真实标签图像（仅 label == 1 的点）
                gt_mask = segment_np == 0
                gt_img = render(x, y, p, gt_mask, self.trainer.cfg.size)

                gt_mask_rain = segment_np == 1
                gt_img_rain = render(x, y, p, gt_mask_rain, self.trainer.cfg.size)

                # 生成预测结果图像（仅 label == 1 的点）
                pred_mask = pred_np == 0
                pred_img = render(x, y, p, pred_mask, self.trainer.cfg.size)

                # 生成预测结果图像（仅 label == 0 的点）
                pred_mask_rain = pred_np == 1 
                pred_img_rain = render(x, y, p, pred_mask_rain, self.trainer.cfg.size)

                # 保存到 TensorBoard
                original_tensor = torch.tensor(original_img).permute(2, 0, 1) / 255.0  # 转为 Tensor
                ...
                if self.trainer.writer is not None:
                    self.trainer.writer.add_image(f"val/original_{i}", original_tensor, self.trainer.epoch)
                    self.trainer.writer.add_image(f"val/ground_truth_{i}", gt_tensor, self.trainer.epoch)
                    self.trainer.writer.add_image(f"val/prediction_{i}", pred_tensor, self.trainer.epoch)
                    self.trainer.writer.add_image(f"val/prediction_rain_{i}", pred_tensor_rain, self.trainer.epoch)
                    self.trainer.writer.add_image(f"val/ground_truth_rain_{i}", gt_img_rain, self.trainer.epoch)
                    # 添加调试信息
            ...
            segment_np = segment.cpu().detach().numpy()
            TP = np.sum((pred_np == 0) & (segment_np == 0))
            TN = np.sum((pred_np == 1) & (segment_np == 1))
            GP = np.sum(segment_np == 0)
            GN = np.sum(segment_np == 1)
            ...
            self.trainer.logger.info(
                info
                + "Loss {loss:.4f} || DA {DA:.4f}".format(
                    iter=i + 1, max_iter=len(self.trainer.val_loader), loss=loss.item(),DA=DA
                )
            )
        loss_avg = self.trainer.storage.history("val_loss").avg
        intersection = self.trainer.storage.history("val_intersection").total
        union = self.trainer.storage.history("val_union").total
        target = self.trainer.storage.history("val_target").total
        DA = self.trainer.storage.history("val_DA").avg
        iou_class = intersection / (union + 1e-10)
        acc_class = intersection / (target + 1e-10)
        m_iou = np.mean(iou_class)
        m_acc = np.mean(acc_class)
    ...
    def after_train(self):
        self.trainer.logger.info(
            "Best {}: {:.4f}".format("DA", self.trainer.best_metric_value)
        )
