...
def _ball_query(sorted_points, original_indices, start, end, x_min, y_min, radius, k):
    n = sorted_points.shape[0]
    radius_sq = radius**2
    results = np.full((n, k), -1, dtype=np.int32)
    max_gx = start.shape[0] - 1
    max_gy = start.shape[1] - 1
    ...
    for i in nb.prange(n):
        ...
        # Calculate the grid where it is located
        gx = int((qx - x_min) // radius)
        gy = int((qy - y_min) // radius)
        
        candidates = []
        # Check the surrounding 3x3 grid
        for dx in (-1, 0, 1):
            for dy in (-1, 0, 1):
                ngx = gx + dx
                ngy = gy + dy
                if ngx < 0 or ngx > max_gx or ngy < 0 or ngy > max_gy:
                    continue
                s = start[ngx, ngy]
                e = end[ngx, ngy]
                if s == -1:
                ...
                # Traverse the points in the grid.
                for j in range(s, e):
                    idx = original_indices[j]
                    if idx == orig_i:  # Exclude oneself
                        continue
                    px, py = sorted_points[j]
                    dx_p = px - qx
                    dy_p = py - qy
        ...
        for pos in range(count):
            results[orig_i, pos] = candidates[pos][1]
    
    return results

def optimized_knn_gather(features: np.ndarray,
                        indices: np.ndarray,
                        fill_value: float = 0.0) -> np.ndarray:
    '''
    Optimized feature aggregation function (memory-friendly + parallel computing)
    The parameters are the same as above, returning the same structured data.
    '''
    # Pre-allocated result array
    N, K = indices.shape
    D = features.shape[1]
    gathered = np.full((N, K, D), fill_value, dtype=features.dtype)
    
    # Batch processing of valid indexes
    valid_mask = indices != -1
    valid_indices = indices[valid_mask]
    gathered[valid_mask] = features[valid_indices]
    
    return gathered

@TRANSFORMS.register_module()
class BallQuery(object):
    def __init__(self, radius=0.1, k = 10) -> None:
        self.radius = radius
        self.k = k
    
    def _get_self_idx(self, k_idx):
        n, k = k_idx.shape
        idx = np.arange(n)[:, None].repeat(k, axis=1)
        return idx
    
    
    def preprocess_points(self, points, radius):
        x = points[:, 0].copy()
        y = points[:, 1].copy()
        ...
        grid_x = ((x - x_min) // radius).astype(np.int32)
        grid_y = ((y - y_min) // radius).astype(np.int32)
        
        # Sort points by grid
        order = np.lexsort((grid_y, grid_x))
        sorted_points = points[order]
        sorted_indices = np.arange(len(points))[order]
        sorted_grid_x = grid_x[order]
        sorted_grid_y = grid_y[order]
        
        # Determine the number of grids
        max_gx = np.max(sorted_grid_x)
        max_gy = np.max(sorted_grid_y)
        start = np.full((max_gx+2, max_gy+2), -1, dtype=np.int32)
        end = np.full_like(start, -1)
        
        current_gx, current_gy = -1, -1
        current_start = 0
        for i in range(len(sorted_grid_x)):
            gx, gy = sorted_grid_x[i], sorted_grid_y[i]
            if gx != current_gx or gy != current_gy:
                if current_gx != -1:
                    start[current_gx, current_gy] = current_start
                    end[current_gx, current_gy] = i
                current_gx, current_gy = gx, gy
                current_start = i
        # Handle the last grid
        if current_gx != -1:
            start[current_gx, current_gy] = current_start
            end[current_gx, current_gy] = len(sorted_grid_x)
        
        return sorted_points, sorted_indices, start, end, (x_min, y_min)
    

    
    def __call__(self, data_dict):
        # print("==== Start BallQuery ===")
        # print(data_dict.keys())
        # print(data_dict['coord'].shape)
        # print(type(data_dict['coord']))
        points = data_dict['coord'][:,:-1]
        sorted_points, sorted_indices, start, end, (x_min, y_min) = self.preprocess_points(points, self.radius)
        idx = _ball_query(sorted_points, sorted_indices, start, end, x_min, y_min, self.radius, self.k)
        idx = np.where(idx == -1, self._get_self_idx(idx), idx)
        data_dict["n_feat"] = points[:,None] - optimized_knn_gather(points,indices=idx)
        # print("==== End BallQuery ===")
        return data_dict
...
class OptimRainRecognize:
    ...
    def __call__(self, data_dict):
        # Convert the data into a numpy array (assuming the data is sorted by time)
        coord = data_dict['coord'].astype(np.float64)
        strength = data_dict['strength'].astype(np.float64)
        
        # Extract data from various dimensions
        x = coord[:, 0]
        y = coord[:, 1]
        t = coord[:, 2]
        polarity = strength[:, 0]
        
        # Pre-calculate the starting indices of all time windows
        j_indices = np.searchsorted(t, t - self.T, side='left')
        # Core calculations accelerated with numba
        p = self._compute_p(x, y, t, polarity, j_indices)
        
        data_dict['p'] = p.reshape(-1, 1)
        # count = 0
        # for _ in data_dict['p']:
        #     if _ > 0: count += 1
        # print(f"Shape:{data_dict['p'].shape} || {data_dict['p'][100:105]} || count {count}")
        return data_dict

    def _compute_p(self, x, y, t, polarity, j_indices):
        # Core computation functions accelerated by Numba
        return self._numba_compute(
            x, y, t, polarity, j_indices,
            self.sx, self.sy, self.alpha
        )

    @staticmethod
    @nb.njit(fastmath=True, parallel=False)
    def _numba_compute(x, y, t, polarity, j_indices, sx, sy, alpha):
        n = len(x)
        p = np.zeros(n)
        ...
        for i in nb.prange(n):
            ...
            # Obtain the time window boundaries
            j = j_indices[i]
            
            # Reverse search for the nearest matching point.
            found = False
            for k in range(i-1, j-1, -1):
                ...
                pol_prod = polarity[i] * polarity[k]
                if pol_prod <= 0:
                    p[i] = alpha * p[k]
                    found = True
                    break
                ...
                slope = (x[i] - x[k]) / (y[k] - y[i])
                if slope < 0.4:
                    p[i] = alpha * p[k] + (1 - alpha)
                else:
                    p[i] = alpha * p[k]
...
class RainRecognize(object):
    ...
    def __call__(self, data_dict):
        ...
        for index in indices:
            x,y,t = coord[index]
            poler = data_dict['strength'][index][0]
            if t < 1e-6:
                p.append(0)
                continue
            # Using the time sequence traversal of points
            for former_index in range(index-1, -1, -1):
                f_x, f_y, f_t = coord[former_index]
                if ((t - f_t) > self.T) or ((t-f_t) < 0):
                    # T not qulify
                    break
                # xy not in STW
                if abs(f_x - x) >= self.sx:
                    continue
                if abs(f_y - y) >= self.sy:
                    continue
                
                f_poler = poler = data_dict['strength'][former_index][0]
                if (poler*f_poler > 0) and (f_y > y) and (((x - f_x)/(y-f_y)) < 0.4):
                    p.append(self.alpha*p[former_index] + (1-self.alpha))
                else: p.append(self.alpha*p[former_index])
                break
                
            if len(p) != (index+1):
                p.append(0)
        
        assert len(p) == len(indices)
        data_dict['p'] = np.array(p).reshape(-1, 1)
        return data_dict
...
class GridQuery(object):
    ...
    def __call__(self, data_dict):
        # print(data_dict.keys()) # dict_keys(['coord', 'strength', 'segment', 'tn', 'p', 'grid_coord'])
        # print(f"grid shape: {data_dict['grid_coord'].shape} || p length : {data_dict['p'].shape}") # (382228, 3)
        # print("===== START ======")
        grid_coord = data_dict['grid_coord']
        coord = data_dict['coord']
        key = self.hash(grid_coord)
        indices = np.arange(grid_coord.shape[0]) 
        mapping = dict(zip(key, indices))
        
        # Generate a 3×3×3 grid with offsets of -1, 0, and 1 in three directions.
        offsets = np.array(np.meshgrid([-1, 0, 1],
                                        [-1, 0, 1],
                                        [-1, 0, 1],
                                        indexing='ij')).T.reshape(-1, 3)
        neighbor_index  = []
        for index in indices:
            center = grid_coord[index]
            neighbor_key = self.hash(center + offsets)
            idx = np.array([mapping.get(k, index) for k in neighbor_key])
            neighbor_index.append(idx)
        
        neighbor_index = np.array(neighbor_index)
        # print(f"coord shape :{data_dict['coord'].shape} || strength shape : {data_dict['strength'].shape} || p shape : {data_dict['p'].shape}")
        # neighbor_feat = np.concatenate((data_dict['coord'],data_dict['strength'], data_dict['p']), axis=1)
        neighbor_feat = np.concatenate((data_dict['coord'],data_dict['strength']), axis=1)
        n_feat = neighbor_feat[:,None] - optimized_knn_gather(neighbor_feat,indices=neighbor_index)
        data_dict['n_feat'] = n_feat
        # print("======END=======")
        return data_dict


@TRANSFORMS.register_module()
class OptimGridQuery(object):
    def __init__(self):
        self.hash = self.fnv_hash_vec_numba  # Hash function accelerated with numba
    
    @staticmethod
    @nb.njit
    def fnv_hash_vec_numba(arr):
        # Accelerated version of FNV64-1A hash function using numba
        h = np.uint64(14695981039346656037)
        hashed_arr = np.zeros(arr.shape[0], dtype=np.uint64)
        for i in nb.prange(arr.shape[0]):
            hv = h
            for j in range(arr.shape[1]):
                hv = hv * np.uint64(1099511628211)
                hv = hv ^ np.uint64(arr[i, j])
            hashed_arr[i] = hv
        return hashed_arr
    
    def __call__(self, data_dict):
        grid_coord = data_dict['grid_coord'].astype(np.int64)  # 确保坐标为整型
        N = grid_coord.shape[0]
        
        # Calculate the hash of the original coordinates and sort them.
        key = self.hash(grid_coord)
        sorted_indices = np.argsort(key)
        sorted_keys = key[sorted_indices]
        
        # Generate all neighboring coordinates
        offsets = np.array(np.meshgrid([-1, 0, 1], [-1, 0, 1], [-1, 0, 1], indexing='ij')).T.reshape(-1, 3)
        neighbor_coords = grid_coord[:, np.newaxis, :] + offsets[np.newaxis, :, :]
        neighbor_coords_flat = neighbor_coords.reshape(-1, 3)
        
        # Calculate neighborhood hash and find index
        neighbor_keys = self.hash(neighbor_coords_flat)
        origin_indices = np.repeat(np.arange(N), 27)
        
        # Use vectorized lookup to replace dictionary queries.
        pos = np.searchsorted(sorted_keys, neighbor_keys)
        pos = np.clip(pos, 0, len(sorted_keys)-1)
        mask = sorted_keys[pos] == neighbor_keys
        neighbor_indices_flat = np.where(mask, sorted_indices[pos], origin_indices)
        neighbor_index = neighbor_indices_flat.reshape(N, 27)
        
        # Calculate feature differences
        neighbor_feat = np.concatenate((data_dict['coord'], data_dict['strength'], data_dict['p']), axis=1)
        gathered_feat = neighbor_feat[neighbor_index]  # 直接使用numpy高级索引
        # current_feat = neighbor_feat[:, np.newaxis, :]
        n_feat = gathered_feat
        data_dict['n_feat'] = n_feat
...
class GridSample(object):
    ...
    def __call__(self, data_dict):
        ...
        scaled_coord = data_dict["coord"] / np.array((1,1,self.grid_size))
    ...
    def __call__(self, data_dict):
        ...
        # mode is "random" or "center"
            ...
            if "p" in data_dict.keys():
                data_dict["p"] = data_dict["p"][idx_crop]
        return data_dict
...
class Normalize(object):
    ...
    def __call__(self, data_dict):
        # Normalize the first few dimensions of data_dict["coord"] and data_dict["segment"].
        if "coord" in data_dict.keys():
            coord = data_dict["coord"]
