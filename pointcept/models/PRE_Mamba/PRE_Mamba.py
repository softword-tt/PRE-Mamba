...
from .mssm import MultiScaleStateSpaceModel
import numpy as np
...
class MultiScaleStateSpaceModelBlock(PointModule):
    def __init__(
        self,
        channels,
        gather_num,
        ...
        self.mamba = MultiScaleStateSpaceModel(d_model=channels, d_conv=5)

    @torch.no_grad()
    def get_padding(self, point):
        offset = point.offset
        bt_bincount = torch.bincount(point.bt.int())
    ...
    def mssm_forward(self, point):
        ...
        # space inverse
        point.feat = point.feat[sp_inverse]        # L, D
        # point.feat.shape ==torch.Size([319468, 16]) torch.Size([405421, 16])
        return point
...
class Block(PointModule):
    def __init__(
        ...
        self.attn = MultiScaleStateSpaceModelBlock(
            channels=channels,
...
class SerializedPooling(PointModule):
    def __init__(
        ...
        # self.n_proj = nn.Linear(in_channels, out_channels)
    ...
    def forward(self, point: Point):
        ...
        point_dict = Dict(
            feat=torch_scatter.segment_csr(
                self.proj(point.feat)[indices], idx_ptr, reduce=self.reduce # self.reduce = max
            ),
...
class STDF(PointModule):
    def __init__(
        ...
        self.stem_final = nn.Sequential(nn.Conv1d(embed_channels, embed_channels, kernel_size=3, padding=1, bias=False),
                                        ...
                                        act_layer())
        self.t_stem = nn.Sequential(nn.Conv1d(1, 1, kernel_size=3, padding=1, bias=False),
                                      act_layer())
    ...
    def forward(self, point: Point, point_tn: Point):
        ...
        pad_t_feat = pad_feat[:,2,:].unsqueeze(1)
        
        pad_feat = self.stem(pad_feat)
        tn1_sparse_feat = self.tn_stem1(pad_tn_feat)
        tn2_sparse_feat = self.tn_stem2(pad_tn_feat)
        t_sparse_feat = self.t_stem(pad_t_feat)
        pad_feat = pad_feat + tn2_sparse_feat + pad_feat * tn1_sparse_feat + pad_feat*t_sparse_feat
...
@MODELS.register_module("PRE_Mamba")
class PRE_Mamba(PointModule):
    def __init__(
        self,
        in_channels=6,
        gather_num=4,
        ...
        self.embedding = self.init_embedding(in_channels=in_channels, enc_channels=enc_channels, bn_layer=bn_layer, act_layer=act_layer)

        self.init_backbone(drop_path, enc_depths, enc_channels, stride, bn_layer, act_layer,
                            gather_num, mlp_ratio, proj_drop, ln_layer, pre_norm, dec_depths, dec_channels)
        
    def init_embedding(self, in_channels, enc_channels, bn_layer, act_layer):
        return STDF(in_channels=in_channels,
    ...
    def init_backbone(self,drop_path, enc_depths, enc_channels, stride, bn_layer, act_layer,
                      gather_num, mlp_ratio, proj_drop, ln_layer, pre_norm, dec_depths, dec_channels):
        # encoder
        enc_drop_path = [
